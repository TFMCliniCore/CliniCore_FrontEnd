...
import { citasApi, pacientesApi, clientesApi, productosApi } from '@/lib/api';
import type { Cita, Producto } from '@/lib/api';
...
export default function DashboardPage() {
  ...
  const [inventario, setInventario] = useState<ProductoCritico[]>([]);
  ...
  const cargarDatos = useCallback(async () => {
    setLoading(true);
    try {
      const [todasCitas, clientes, mascotas, productos] = await Promise.allSettled([
        citasApi.listar(),
        clientesApi.listar(),
        pacientesApi.listar(),
        productosApi.listar({ stockBajo: true }),
      ]);

      console.log('CLIENTES:', clientes.status, clientes.status === 'fulfilled' ? clientes.value.length : (clientes as any).reason?.message);
      console.log('MASCOTAS:', mascotas.status, mascotas.status === 'fulfilled' ? mascotas.value.length : (mascotas as any).reason?.message);
      console.log('CITAS:', todasCitas.status, todasCitas.status === 'fulfilled' ? todasCitas.value.length : (todasCitas as any).reason?.message);
      console.log('PRODUCTOS:', productos.status, productos.status === 'fulfilled' ? productos.value.length : (productos as any).reason?.message);

      // ── KPI Clientes ──────────────────────────────────────
      const totalClientes = clientes.status === 'fulfilled'
        ? clientes.value.length
        : 1284;

      // ── KPI Mascotas ──────────────────────────────────────
      const totalMascotas = mascotas.status === 'fulfilled'
        ? mascotas.value.length
        : 842;

      // ── KPI Citas hoy ─────────────────────────────────────
      const listaCitas = todasCitas.status === 'fulfilled' ? todasCitas.value : [];
      const hoy = new Date().toDateString();
      const citasDeHoy = listaCitas.filter((c) => new Date(c.fecha).toDateString() === hoy);

      setKpi({
        clientes: totalClientes,
        mascotas: totalMascotas,
        ventasMes: 45200000, // Sin MS Ventas aún
        citasHoy: citasDeHoy.length,
      });

      setCitas(citasDeHoy.slice(0, 5));

      // ── Inventario crítico ────────────────────────────────
      if (productos.status === 'fulfilled') {
        const criticos: ProductoCritico[] = productos.value
          .filter((p: Producto) => p.cantidadActual < p.cantidadMinima)
          .slice(0, 6)
          .map((p: Producto) => ({
            id: p.id,
            nombre: p.nombre,
            ...
            stockMinimo: p.cantidadMinima,
            estado: p.cantidadActual <= Math.floor(p.cantidadMinima / 2) ? 'critico' : 'bajo',
          }));
        setInventario(criticos);
      }

      setLastUpdate(new Date());
    } catch {
      // silencioso — cada sección maneja su propio fallback
    } finally {
      setLoading(false);
    }
  }, []);
              ...
              return (
                <div key={cita.id} className="flex items-center gap-3 p-3 rounded-xl border border-transparent hover:border-gray-100 hover:bg-gray-50 transition-all cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 truncate">{cita.motivo} · {formatHora(cita.fecha)}</p>
                  </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                {loading ? 'Cargando...' : inventario.length === 0 ? 'Sin productos bajo mínimo' : `${inventario.length} productos bajo mínimo`}
              </p>
            </div>
          </div>
        ...
        {loading ? (
          <div className="p-6 space-y-3">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="animate-pulse h-10 bg-gray-50 rounded-xl" />
            ))}
          </div>
        ) : inventario.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-gray-400">
            <CheckCircle2 size={32} className="mb-2 text-emerald-400 opacity-60" />
            <p className="text-sm font-medium text-emerald-600">Todo el inventario está en niveles normales</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-gray-50">
                {inventario.map((prod) => (
                  <tr key={prod.id} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <Link href={`/productos/${prod.id}`} className="opacity-0 group-hover:opacity-100 text-xs font-semibold text-teal-600 hover:text-teal-700 transition-all flex items-center gap-1">
