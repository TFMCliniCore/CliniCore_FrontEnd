...
import { useEffect, useState, useCallback, useRouter } from "react";
import {
  Users,
  Stethoscope,
  CheckCircle2,
...
const ROLES_RESUMEN = [
  {
    rol: "Administrador",
    desc: "Acceso total a finanzas y configuración",
    count: 4,
    color: "text-violet-600",
  },
  {
    rol: "Veterinario",
    desc: "Gestión médica, farmacia e historias clínicas",
    count: 12,
    color: "text-blue-600",
  },
  {
    rol: "Recepcionista",
    desc: "Citas, POS y gestión básica de clientes",
    count: 26,
    color: "text-gray-600",
  },
];
...
const ACTIVIDAD = [
  {
    texto: "Alejandro Ruiz editó el perfil de Carlos Méndez",
    tiempo: "Hace 12 min",
    color: "bg-blue-500",
  },
  {
    texto: "Nuevo usuario creado: mfernanda_adm",
    tiempo: "Hace 2 horas",
    color: "bg-violet-500",
  },
  {
    texto: "Sistema generó reporte de auditoría de accesos",
    tiempo: "Ayer 18:45",
    color: "bg-gray-400",
  },
];
...
const MOCK_USUARIOS: UsuarioExtended[] = [
  ...
  },
  {
    ...
    email: "mfernanda@clinica.com",
    cargo: "Administración Central",
    username: "mfernanda_adm",
    rol: "Admin",
  ...
  },
  {
    ...
    email: "cmendez@clinica.com",
    cargo: "Atención al Cliente",
    username: "cmendez_recep",
    rol: "Recepcionista",
    sucursal: "Sede Sur",
    estado: "inactivo",
  },
  ...
  {
    id: 5,
    nombres: "Pedro Villamizar",
    email: "pvillamizar@clinica.com",
    cargo: "Farmacéutico",
    username: "pvillamizar",
    rol: "Veterinario",
    sucursal: "Sede Norte",
    estado: "activo",
  },
  {
    id: 6,
    nombres: "Sandra Ospina",
    email: "sospina@clinica.com",
...
function ModalUsuario({
  ...
  open: boolean;
  onClose: () => void;
  onSave: (u: Partial<UsuarioExtended>) => void;
}) {
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              >
                {["Veterinario", "Admin", "Recepcionista", "Superadmin"].map(
                  (r) => (
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-teal-400 bg-gray-50 transition-all"
              >
                {["Sede Norte", "Sede Central", "Sede Sur", "Todas"].map(
                  (s) => (
...
export default function UsuariosPage() {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      const ext: UsuarioExtended[] = data.length
        ? data.map((u, i) => ({
            ...u,
            username: u.email.split("@")[0],
            rol: (
              ["Veterinario", "Admin", "Recepcionista", "Superadmin"] as Rol[]
            )[i % 4],
  ...
  const stats = {
    ...
    admins: usuarios.filter((u) => u.rol === "Admin" || u.rol === "Superadmin")
      .length,
  };
    ...
    <div className="space-y-6 pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Administra accesos, roles y permisos del personal.
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={cargar}
            className="p-2 rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-50 hover:text-teal-600 transition-colors"
          >
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 flex flex-wrap items-center gap-3">
        ...
        >
          ...
          {["Veterinario", "Admin", "Recepcionista", "Superadmin"].map((r) => (
            <option key={r}>{r}</option>
          ))}
        </select>
        <select
          value={sucFiltro}
          onChange={(e) => {
            setSucFiltro(e.target.value);
            setPagina(1);
          }}
          className="px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-teal-400 transition-all bg-white text-gray-700"
        >
          <option value="">Todas las Sucursales</option>
          {["Sede Norte", "Sede Central", "Sede Sur", "Todas"].map((s) => (
            <option key={s}>{s}</option>
          ))}
        </select>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                ...
                : pagActual.map((u) => (
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-10 h-10 rounded-xl bg-gradient-to-br ${avatarColor(u.id)} flex items-center justify-center text-white text-xs font-bold flex-shrink-0`}
                          >
                            {initials(u.nombres)}
                          </div>
                          <div>
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`text-[11px] font-bold px-2.5 py-1 rounded-full border ${ROL_BADGE[u.rol]}`}
                        >
                          {u.rol}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div
                          className={`flex items-center gap-1.5 text-xs font-semibold ${u.estado === "activo" ? "text-emerald-600" : "text-gray-400"}`}
                        >
                          <span
                            className={`w-2 h-2 rounded-full ${u.estado === "activo" ? "bg-emerald-500" : "bg-gray-300"}`}
                          />
        ...
        <div className="px-6 py-4 flex items-center justify-between border-t border-gray-100 bg-gray-50">
          <p className="text-xs text-gray-500">
            Mostrando {Math.min((pagina - 1) * PER_PAGE + 1, filtrados.length)}–
            {Math.min(pagina * PER_PAGE, filtrados.length)} de{" "}
          ...
          <div className="flex items-center gap-1.5">
            ...
            {Array.from({ length: totalPag }).map((_, i) => (
              <button
                ...
                style={
                  pagina === i + 1
                    ? { background: "linear-gradient(135deg,#0e314d,#0a8661)" }
                    : {}
                }
              >
                {i + 1}
          ...
          <div className="space-y-3">
            {ROLES_RESUMEN.map((r) => (
              ...
              >
                ...
                <span
                  className={`text-xs font-mono font-bold px-2.5 py-1 rounded-lg bg-white border border-gray-200 ${r.color}`}
                >
                  {r.count} USR
                </span>
              </div>
          ...
          <ul className="space-y-5 relative">
            {ACTIVIDAD.map((a, i) => (
              <li key={i} className="flex gap-4">
                <div
                  className={`w-2 h-2 rounded-full ${a.color} mt-1.5 flex-shrink-0`}
                />
                <div>
                  <p
                    className="text-sm text-gray-700"
                    dangerouslySetInnerHTML={{
                      __html: a.texto.replace(
                        /([A-Z][a-záéíóú]+ [A-Z][a-záéíóú]+)/g,
                        '<span class="font-bold text-gray-900">$1</span>',
                      ),
                    }}
                  />
                  <p className="text-[11px] text-gray-400 uppercase tracking-wide mt-1">
                    {a.tiempo}
                  </p>
                </div>
              </li>
